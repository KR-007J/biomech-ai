...
class InverseKinematicsSolver:
    ...
    def _initialize_body_model(self) -> Dict[str, Joint]:
        ...
        return {
            ...
            "left_knee": Joint("left_knee", np.array([-0.1, -0.5, 0]), "left_hip", min_angle=0, max_angle=140, mass=2.0),
            "right_knee": Joint("right_knee", np.array([0.1, -0.5, 0]), "right_hip", min_angle=0, max_angle=140, mass=2.0),
            "left_ankle": Joint("left_ankle", np.array([-0.1, -0.9, 0]), "left_knee", min_angle=-30, max_angle=30, mass=1.5),
            "right_ankle": Joint("right_ankle", np.array([0.1, -0.9, 0]), "right_knee", min_angle=-30, max_angle=30, mass=1.5),
            ...
            "left_elbow": Joint("left_elbow", np.array([-0.3, 0.2, 0]), "left_shoulder", min_angle=0, max_angle=160, mass=1.2),
            "right_elbow": Joint("right_elbow", np.array([0.3, 0.2, 0]), "right_shoulder", min_angle=0, max_angle=160, mass=1.2),
        }
...
class MuscleActivationModel:
    ...
    MUSCLE_REFERENCE = {
        ...
        "tibialis_anterior": {"max_force": 800, "optimal_angle": 20},  # Ankle dorsiflexion
        ...
        try:
            ...
            for muscle_name, ref in self.MUSCLE_REFERENCE.items():
                ...
                if relevant_angle is not None:
                    ...
                    self.muscle_fatigue[muscle_name] = min(1.0, 
                        self.muscle_fatigue[muscle_name] + fatigue_increment)
