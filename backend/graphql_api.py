        ...
        try:
            ...
            for field in mutation.get("fields", []):
                if field_def := self.schema.mutation_type.fields.get(field):
                    value = await self.resolver.resolve_field(
                        self.schema.mutation_type.name,
                        field,
                        None,
                        variables or {}
                    )
...
class GraphQLServer:
    ...
    def execute_query(self, query: Optional[str] = None, variables: Optional[Dict[str, Any]] = None, operation_name: Optional[str] = None, query_string: Optional[str] = None) -> Dict[str, Any]:
